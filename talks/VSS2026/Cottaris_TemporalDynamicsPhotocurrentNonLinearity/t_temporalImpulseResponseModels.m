...
%   t_temporalImpulseResponseModels()
    ...
    fancyPlotImpulseResponse(...
        ...
        'spot-only', ...
        exportVisualizationRootDirectory, ...
        sprintf('BK_Fig6_ON_spot_only.pdf'));
     ...
     fancyPlotImpulseResponse(...
        ...
        'annulus-only', ...
        exportVisualizationRootDirectory, ...
        sprintf('BK_Fig6_ON_annulus_only.pdf'));
...
function fancyPlotImpulseResponse(theCenterIRdata, theSurroundIRdata, theCenterColor, theSurroundColor, ...
    whatToPlot, exportVisualizationRootDirectory, thePDFfileName)
    ...
    pHandles = [];
    theLegends = {};
    if (strcmp(whatToPlot, 'both')) || (strcmp(whatToPlot, 'spot-only'))
        pHandles(numel(pHandles)+1) = scatter(ax, theCenterIRdata.temporalSupportSeconds*1e3, theCenterIRdata.amplitude/maxAmplitude, 12*12, ...
                    'LineWidth', 1.5, ...
                    'MarkerFaceColor', theCenterColor, ...
                    'MarkerFaceAlpha', 1.0, 'MarkerEdgeColor', [1 1 1], 'MarkerEdgeAlpha', 0.5 );
        theLegends{numel(theLegends)+1} = 'spot';
    end
       
    hold(ax, 'on');

    if (strcmp(whatToPlot, 'both')) || (strcmp(whatToPlot, 'annulus-only'))
        pHandles(numel(pHandles)+1) = scatter(ax, theSurroundIRdata.temporalSupportSeconds*1e3, theSurroundIRdata.amplitude/maxAmplitude, 12*12, ...
                'LineWidth', 1.5, ...
                'MarkerFaceColor', theSurroundColor, ...
                'MarkerFaceAlpha', 1.0, 'MarkerEdgeColor', [1 1 1], 'MarkerEdgeAlpha', 0.5 );
        theLegends{numel(theLegends)+1} = 'annulus';
    end


    legend(ax, pHandles, theLegends, 'Location', 'NorthEast', 'NumColumns', 1);
    yAxisLims = 1.01*[-1 1];
    yAxisTicks = -1:0.5:1;
    xAxisLims = [0 200];
    xAxisTicks = 0:25:250;
