...
%       of the photocurrentBasedTTF * intrinscicTTF  = BenardeteKaplan1997TTF (separately for the center and the surround)
...
%{
    ...
    % Specify a smaller # of wavefront samples (usually we employ 501).
    ...
    % No mRGC non-linearity at this point
    mRGCNonLinearityParamsStruct = {};


	% ----- Compute input cone mosaic EXCITATION & PHOTOCURRENTSresponses (SPOT stimulus) ------
	t_mRGCMosaicSynthesizeTemporalFilters(...
       ...
       'meanLuminanceCdM2', backgroundLuminanceCdM2, ...
       'backgroundXYchromaticity', backgroundChromaticity, ...
    ...
    % -----  Compute input cone mosaic EXCITATION & PHOTOCURRENT responses (ANNULUS stimulus)  ----
	t_mRGCMosaicSynthesizeTemporalFilters(...
       'rgcMosaicName', 'JCNpaperTemporal7DegsMosaic', ...
       'opticsWavefrontSpatialSamples', opticsWavefrontSpatialSamples, ...
       'stimulusPixelSizeAsFractionOfConeAperture', stimulusPixelSizeAsFractionOfConeAperture, ...
    ...
    % -------Only inspect computed input cone mosaic EXCITATION & PHOTOCURRENT responses (SPOT STIMULUS) -----
    ...
    % -------Only inspect computed input cone mosaic EXCITATION & PHOTOCURRENT responses (ANNULUS STIMULUS) -----
    ...
    % -----  Compute mRGC mosaic responses (SPOT stimulus) ----
    computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated = true;
    computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated = true;
    ...
    t_mRGCMosaicSynthesizeTemporalFilters(...
       ...
       'computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex', [30], ...
       'computeMRGCMosaicResponses', true, ...
       'computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated', computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated, ...
       'computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated', computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated);

    % -----  Compute mRGC mosaic responses (ANNULUS stimulus) ----
    ...
    % -----  Parameters for synthesizing the temporal filters

    %temporalFilterSynthesisMethod = 'dampedOscillationFilter';
    %temporalFilterSynthesisMethod = 'dampedOscillationLowPassCascadeFilter';
    %temporalFilterSynthesisMethod = 'differenceOfLowPassFilters';
    temporalFilterSynthesisMethod = 'direct division of TTFs';

    innerRetinaFilterDerivationParams = struct(...
        'finiteTimeIRdurationSeconds', 150/1000, ...
        'leftFiniteDurationWindowDurationSeconds', 7.5/1000, ...
        'rightFiniteDurationWindowDurationSeconds', 50/1000, ...
        'temporalFilterSynthesisMethod', temporalFilterSynthesisMethod, ...
        'amplitudeSpectrumVsComplexSpectrumBias', 0.0, ...   % 0 .. 1
        'minFrequencyHzWithNonZeroWeight', 0.5, ...
        'maxFrequencyHzWithNonZeroWeight', 100, ...
        'minTimeDelaySecondsWithUnitWeight', 0/1000, ...
        'maxTimeDelaySecondsWithUnitWeight', 100/1000, ...
        'timeDomainResidualWeighting', 0.0, ...
    ...
    targetCellCenterImpulseResponseSource = 'Benardete&Kaplan 1997, Figure 6 (ON), center';
    targetCellSurroundImpulseResponseSource = 'Benardete&Kaplan 1997, Figure 6 (ON), surround';


   
    % -----  First synthesize the center mRGC temporal filter (it is
    %        cleaner than the surround)
    ...
    t_mRGCMosaicSynthesizeTemporalFilters(...
        ...
        'innerRetinaFilterDerivationParams', innerRetinaFilterDerivationParams, ...
        'targetCellImpulseResponseSource', targetCellCenterImpulseResponseSource, ...
        'computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated', computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated, ...
        'computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated', computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated, ...
        'onlyVisualizePreviouslySynthesizedFilters', true);


    % -----  Finally synthesize the surround mRGC temporal filter from the
    % annulus data (it must have contributions from the center IR)
    ...
    t_mRGCMosaicSynthesizeTemporalFilters(...
        ...
        'synthesizeMRGCtemporalFiltersForRGCwithIndex', [30], ...
        'innerRetinaFilterDerivationParams', innerRetinaFilterDerivationParams, ...
        'targetCellImpulseResponseSource', targetCellSurroundImpulseResponseSource, ...
        'computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated', computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated, ...
        'computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated', computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated, ...
        'onlyVisualizePreviouslySynthesizedFilters', true);
...
arguments
    ...
    % How to compute phase
    % set this to [] to compute phase from fitted sinusoid. This assumes that the response data are time aligned with the stimulus. 
    % or set it to some delay value, specified in milliseconds (for the photocurrent response, this is 40-50 msec)
    options.delayMsecondsForPhaseComputation (1,:) = 0;  % compute phase using a pure delay of the photocurrent impulse response, (usually 40-50 msec)


    % Struct with info on how to derive the inner retina temporal filter
    options.innerRetinaFilterDerivationParams = [];

    % Target cell impulse response function
    options.targetCellImpulseResponseSource (1,:) char{mustBeMember(options.targetCellImpulseResponseSource, ...
        {...
            'Benardete&Kaplan 1997, Figure 6 (ON), center' ...
            'Benardete&Kaplan 1997, Figure 6 (ON), surround' ...
            'Benardete&Kaplan 1997, Figure 6 (OFF), center' ...
            'Benardete&Kaplan 1997, Figure 6 (OFF), surround' ...
            'Benardete&Kaplan 1997, Figure 7, center' ...
            'Benardete&Kaplan 1997, Figure 7, surround' ...
        })} = 'Benardete&Kaplan 1997, Figure 6 (ON), center';
    ...
    options.visualizeSinusoidalFits (1,1) logical = false;
    ...
    options.computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated(1,1) logical = false;
    options.computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated(1,1) logical = false;

    options.synthesizeMRGCtemporalFiltersForRGCwithIndex (1,:) double = [];
    options.onlyVisualizePreviouslySynthesizedFilters (1,1) logical = false;

    % Debug the computation of one stimulus period of photocurrent responses
    options.debugInputConeMosaicPcurrentResponse (1,1) logical = false;
...
% mRGC response phase delay computation
delayMsecondsForPhaseComputation = options.delayMsecondsForPhaseComputation;

% Struct with params for deriving the inner retinal temporal filter
innerRetinaFilterDerivationParams = options.innerRetinaFilterDerivationParams;

% Target cell and center/surround filter
targetCellImpulseResponseSource = options.targetCellImpulseResponseSource;
...
visualizeSinusoidalFits = options.visualizeSinusoidalFits;
...
computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex = options.computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex;
computeMRGCMosaicResponses = options.computeMRGCMosaicResponses;
computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated = options.computeMRGCMosaicResponsesAlsoWithCenterMechanismDeactivated;
computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated = options.computeMRGCMosaicResponsesAlsoWithSurroundMechanismDeactivated;


synthesizeMRGCtemporalFiltersForRGCwithIndex = options.synthesizeMRGCtemporalFiltersForRGCwithIndex;
onlyVisualizePreviouslySynthesizedFilters = options.onlyVisualizePreviouslySynthesizedFilters;
...
if (~isempty(extraInfoEncodedInFileName))
    postFix = sprintf('%s_%s_%s@%2.1fC_%2.0fCdM2_%s_Ecc_%2.1f_%2.1f_Size_%2.1f_%2.1f', ...
	    stimulusShape, ...
...
shortTargetCellImpulseResponseSource = strrep(strrep(strrep(strrep(strrep(strrep(targetCellImpulseResponseSource, 'Benardete&Kaplan 1997, ', ''), ' ', '_'), ',', ''), 'surround', 'srnd'), 'center', 'cnt'), 'Figure_', 'Fig');

theAnalyzedTTFsFileName = fullfile(TTFsubDir, sprintf('IR%d_%s_%s_%s.mat', synthesizeMRGCtemporalFiltersForRGCwithIndex, shortTargetCellImpulseResponseSource, '???', postFix));
...
if (~isempty(mRGCNonLinearityParamsStruct))
    theInputConeMosaicTTFResponsesFullFileName = ...
        strrep(theInputConeMosaicTTFResponsesFullFileName, '.mat', sprintf('%s.mat', mRGCNonLinearityParamsStruct.label));
    theMRGCMosaicTTFResponsesFullFileName = ...
        strrep(theMRGCMosaicTTFResponsesFullFileName, '.mat', sprintf('%s.mat', mRGCNonLinearityParamsStruct.label));
    theAnalyzedTTFsFullFileName = ...
        strrep(theAnalyzedTTFsFullFileName, '.mat', sprintf('%s.mat', mRGCNonLinearityParamsStruct.label));
...
if (computeInputConeMosaicResponses)
	...
	RGCMosaicAnalyzer.compute.mosaicTTFsForStimulusChromaticityAndOptics(...
        ...
        'computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex', computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex, ...
...
if (computeMRGCMosaicResponses || inspectInputConeMosaicResponses || visualizeMosaicResponses)
    ...
    RGCMosaicAnalyzer.compute.mosaicTTFsForStimulusChromaticityAndOptics(...
        ...
        'computeInputConeMosaicResponsesBasedOnPhotocurrents', false, ...
        'computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex', computePhotocurrentResponsesOnlyForInputsToSingleRGCwithIndex, ...
        'inspectInputConeMosaicResponses', inspectInputConeMosaicResponses, ...
...
% Derive the inner retinal temporal filters that the computed photocurrentsBasedTTF
% when deconvolved with gives the impulse response specified by the targetCellImpulseResponseSource
if (~isempty(synthesizeMRGCtemporalFiltersForRGCwithIndex))
    ...
    RGCMosaicAnalyzer.compute.MRGCtemporalFiltersFromPhotocurrentsBasedTTF(...
            ...
            'importSurroundTTFtoBackOut', false, ...
            'importCenterTTFtoBackOut', false);
%
end
